...
| `pkg/inventory` | **Method specified.** Serial nested class authorisations are the textbook leg-based inventory control, EMSR-b (Belobaba; Talluri and van Ryzin ch. 2) sets them from a forecast, and additive leg bid prices from those ladders give the network control over connecting itineraries; the numbers are the caller's. The EMSR-b tests check against the normal table, not the code. |
